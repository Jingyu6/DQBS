...
def worker(seed, beta):
	...
	print(
		'\nEnv Name: %s | Seed: %d | State Dim: %d | Action Dim: %d | Beta: %s has starts.'
		% (ENV_NAME, seed, STATE_DIM, ACTION_DIM, beta)
		)
	...
	et = time.time()
	print(
		'\nEnv Name: {} | Seed: {} | State Dim: {} | Action Dim: {} | Beta: {} has finished, elapsed time: {:2.4f}s.'
			.format(ENV_NAME, seed, STATE_DIM, ACTION_DIM, beta, et - st)
		)
...
def main():
	...
	for beta in BETAS:
		for seed in SEED_LIST:
			arguments.append([seed, beta])

	with Pool(PARAMS['num_workers']) as p:
		return_results = p.starmap(worker, arguments)

	for (_, beta), record in zip(arguments, return_results):
		if beta not in records:
			records[beta] = []
		records[beta].append(record)

		records_file = open('result/beta/record_beta_{}.pickle'.format(ENV_NAME), 'wb')
		pickle.dump(records, records_file)
		records_file.close()


def plot_beta_by_env():
	with open('result/beta/record_beta_{}.pickle'.format(ENV_NAME), 'rb') as file:
		records = pickle.load(file)

	for beta in BETAS:
		data = np.array(records[beta])
		print(data)
	...
	plt.legend(["beta = {}".format(beta) for beta in BETAS])
	plt.savefig(os.path.join('result/beta/', 'Beta_{}.svg'.format(ENV_NAME)))
	plt.clf()

...
ENV_NAME = "Acrobot-v1"#"CartPole-v0" #"MountainCar-v0"
...
PARAMS["use_prioritized_buffer"] = True
...
def worker(seed, alpha):
	...
	print(
		'\nEnv Name: %s | Seed: %d | State Dim: %d | Action Dim: %d | Alpha: %s has starts.'
		% (ENV_NAME, seed, STATE_DIM, ACTION_DIM, alpha)
		)
	...
	et = time.time()
	print(
		'\nEnv Name: {} | Seed: {} | State Dim: {} | Action Dim: {} | Alpha: {} has finished, elapsed time: {:2.4f}s.'
			.format(ENV_NAME, seed, STATE_DIM, ACTION_DIM, alpha, et - st)
		)
...
def main():
	...
	for alpha in ALPHAS:
		for seed in SEED_LIST:
			arguments.append([seed, alpha])

	with Pool(PARAMS['num_workers']) as p:
		return_results = p.starmap(worker, arguments)

	for (_, alpha), record in zip(arguments, return_results):
		if alpha not in records:
			records[alpha] = []
		records[alpha].append(record)

		records_file = open('result/alpha/record_alpha_{}.pickle'.format(ENV_NAME), 'wb')
		pickle.dump(records, records_file)
		records_file.close()


def plot_alpha_by_env():
	with open('result/alpha/record_alpha_{}.pickle'.format(ENV_NAME), 'rb') as file:
		records = pickle.load(file)

	for alpha in ALPHAS:
		data = np.array(records[alpha])
		print(data)
	...
	plt.legend(["alpha = {}".format(alpha) for alpha in ALPHAS])
	plt.savefig(os.path.join('result/alpha/', 'Alpha_{}.svg'.format(ENV_NAME)))
	plt.clf()

...
def worker(seed, backstep):
	...
	for i_episode in range(NUM_EPOCHS):
		...
		if i_episode % LOG_INTERVAL == 0:
			...
			if SET_VERBOSE:
				print(
					'Algo: {}, Seed: {}, Episode {}\tLast reward: {:.2f}\tRunning training reward: {:.2f}\tEvaluation reward: {:.2f}\tRunning eval reward: '.format(
...
def main():
	...
	for backstep in BACKSTEPS:
		records[backstep] = [[] for _ in range(len(SEED_LIST))]
	...
	for (_, backstep), record in zip(arguments, return_results):
		...
		records_file = open('record_backstep_{}.pickle'.format(ENV_NAME), 'wb')
...
def plot_backstep_by_env():
	with open('record_backstep_{}.pickle'.format(ENV_NAME), 'rb') as file:
		records = pickle.load(file)

	for backstep in BACKSTEPS:
		data = records[backstep]

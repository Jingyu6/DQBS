...
def main():
	...
	for (_, backstep), record in zip(arguments, return_results):
		...
		records_file = open('result/backstep/pri/record_backstep_{}.pickle'.format(ENV_NAME), 'wb')
		pickle.dump(records, records_file)
		records_file.close()


def plot_backstep_by_env():
	with open('result/backstep/pri/record_backstep_{}.pickle'.format(ENV_NAME), 'rb') as file:
	...
	plt.savefig(os.path.join('result/backstep/pri/', 'Backstep_{}.svg'.format(ENV_NAME)))
	plt.clf()
